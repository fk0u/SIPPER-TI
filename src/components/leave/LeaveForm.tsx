      ...
      <form onSubmit={handleSubmit} className="doppelrand-core p-6 sm:p-8 space-y-6">
        ...
        <div className="border-b border-white/5 pb-4">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[10px] font-mono uppercase tracking-[0.2em] text-blue-400 font-semibold">
              Portal Pengajuan Presensi
            </span>
          </div>
          <h2 className="text-xl sm:text-2xl font-bold tracking-tight text-white flex items-center gap-2.5">
            <Calendar className="w-5 h-5 text-blue-400" /> Formulir Izin Kuliah
          </h2>
          ...
          <div className="bg-slate-950/70 p-4 sm:p-5 rounded-2xl border border-white/5 space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <label className="text-xs font-semibold text-white">Mode Pengajuan (Proxy)</label>
              </div>
              <button
                type="button"
                onClick={() => {
                ...
                className={`px-3.5 py-1 rounded-full text-xs font-medium transition active:scale-[0.98] ${
                  isProxy
            ...
            {isProxy && (
              <div className="space-y-1.5 pt-2 border-t border-white/5 animate-in fade-in">
                ...
                <select
                  ...
                  className="w-full bg-slate-900 border border-purple-500/40 rounded-xl px-3.5 py-2 text-xs text-white focus:outline-none focus:border-purple-400"
                >
        ...
        <div className="space-y-1.5">
          ...
          <select
            ...
            className="w-full bg-slate-950 border border-white/10 rounded-2xl px-3.5 py-2.5 text-xs text-white focus:outline-none focus:border-blue-500 shadow-inner"
          >
            {courses.map((c) => (
          ...
          <div className="grid grid-cols-3 gap-2">
            {(['sakit', 'izin', 'acara'] as LeaveType[]).map((type) => (
              <button
                ...
                className={`py-2.5 px-3 rounded-2xl text-xs font-medium capitalize transition active:scale-[0.98] text-center ${
                  leaveType === type
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            {diffDays > 0 && (
              <span className="text-[11px] font-semibold text-blue-400 bg-blue-500/10 px-2.5 py-0.5 rounded-full border border-blue-500/20 font-mono">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="space-y-1">
              <span className="text-[10px] text-slate-400 font-mono">Tanggal Mulai:</span>
              <input
                type="date"
                value={startDate}
                onChange={(e) => {
                ...
                required
                className="w-full bg-slate-950 border border-white/10 rounded-2xl px-3.5 py-2 text-xs text-white focus:outline-none focus:border-blue-500 font-mono shadow-inner"
              />
            </div>
            <div className="space-y-1">
              <span className="text-[10px] text-slate-400 font-mono">Tanggal Selesai:</span>
              <input
                type="date"
                value={endDate}
                min={startDate}
        ...
        {/* 5. Reason Text */}
        <div className="space-y-1.5">
          <label className="text-xs font-semibold text-slate-300 block">Keterangan / Alasan Rinci</label>
          <textarea
            value={reason}
            ...
            className="w-full bg-slate-950 border border-white/10 rounded-2xl p-3.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-blue-500 shadow-inner"
          />
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="relative border-2 border-dashed border-white/10 hover:border-blue-500/50 rounded-2xl p-6 text-center bg-slate-950/40 transition group cursor-pointer">
            <input
              type="file"
            ...
            <div className="flex flex-col items-center justify-center space-y-2">
              <div className="p-3 bg-blue-600/10 text-blue-400 rounded-full group-hover:scale-110 transition-transform">
          ...
          {attachments.length > 0 && (
            <div className="space-y-2 pt-2">
              {attachments.map((att, idx) => (
                <div
                  key={idx}
                  className="flex items-center justify-between px-3.5 py-2.5 bg-slate-950 rounded-2xl border border-white/5 text-xs"
                >
        ...
        {/* Submit Button with Nested Icon */}
        <button
          type="submit"
          disabled={isSubmitting || isSuccess}
          className="group w-full py-3 pl-6 pr-3 bg-blue-600 hover:bg-blue-500 text-white font-semibold text-xs sm:text-sm rounded-full shadow-lg shadow-blue-900/30 transition-all disabled:opacity-50 flex items-center justify-between active:scale-[0.98]"
        >
