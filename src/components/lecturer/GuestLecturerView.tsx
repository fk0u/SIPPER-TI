      ...
      <div className="min-h-[70vh] flex items-center justify-center p-4">
        <div className="doppelrand-shell max-w-md w-full">
          <div className="doppelrand-core p-8 text-center space-y-4">
            ...
            <div className="p-3 bg-slate-950 rounded-2xl border border-white/5 text-[11px] font-mono text-slate-500">
              Token: {tokenString}
            </div>
          </div>
        </div>
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto pb-16">
        ...
        <div className="doppelrand-core p-6 sm:p-8 space-y-5">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-white/5 pb-4">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-2xl bg-blue-600/20 text-blue-400 border border-blue-500/30 flex items-center justify-center print:hidden shadow-inner">
                <GraduationCap className="w-6 h-6" />
              </div>
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-[9px] font-mono font-semibold bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 px-2.5 py-0.5 rounded-full">
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  Dosen Pengampu: <strong className="text-slate-200 font-medium">{assignedCourse?.lecturer_name || 'Dosen Pengampu TI Intl'}</strong>
                </p>
              </div>
            </div>
            ...
            <button
              onClick={handlePrint}
              className="px-5 py-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-full text-xs font-semibold shadow-lg shadow-blue-900/30 transition flex items-center gap-2 print:hidden active:scale-[0.98]"
            >
              <Printer className="w-4 h-4" /> Cetak Rekap Presensi
            </button>
          </div>
          ...
          {assignedCourse && (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
              <div className="bg-slate-950/70 p-3 rounded-2xl border border-white/5">
                <span className="text-slate-400 block text-[10px] font-mono uppercase">Jadwal Kuliah</span>
                <span className="text-white font-medium text-xs mt-0.5 block">{assignedCourse.day_of_week}</span>
              </div>
              <div className="bg-slate-950/70 p-3 rounded-2xl border border-white/5">
                <span className="text-slate-400 block text-[10px] font-mono uppercase">Waktu Perkuliahan</span>
                <span className="text-white font-medium text-xs mt-0.5 block">{assignedCourse.start_time} - {assignedCourse.end_time} WITA</span>
              </div>
              <div className="bg-slate-950/70 p-3 rounded-2xl border border-white/5">
                <span className="text-slate-400 block text-[10px] font-mono uppercase">Ruang Kelas</span>
                <span className="text-white font-medium text-xs mt-0.5 block">{assignedCourse.room}</span>
              </div>
              <div className="bg-slate-950/70 p-3 rounded-2xl border border-white/5">
                <span className="text-slate-400 block text-[10px] font-mono uppercase">Semester Aktif</span>
                <span className="text-white font-medium text-xs mt-0.5 block">{assignedCourse.semester}</span>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="grid grid-cols-3 gap-3 sm:gap-4 print:hidden">
        <div className="bg-slate-900/60 border border-white/5 p-4 rounded-3xl">
        ...
        <div className="bg-slate-900/60 border border-white/5 p-4 rounded-3xl">
          <span className="text-xs text-slate-400 block mb-1">Dispensasi Acara</span>
      ...
      <div className="bg-slate-900/70 border border-white/5 p-4 rounded-3xl flex flex-col sm:flex-row gap-3 print:hidden shadow-xl">
        ...
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            className="bg-slate-950 border border-white/10 rounded-full px-4 py-2 text-xs text-white focus:outline-none focus:border-blue-500 font-mono shadow-inner"
          />
          {selectedDate && (
            <button
              ...
              <tbody className="divide-y divide-white/5 font-sans">
                {displayedLeaves.map((req) => (
                  <tr key={req.id} className="hover:bg-slate-900/40 transition">
                    ...
                    <td className="px-5 py-4">
                      <span className={`px-2.5 py-0.5 rounded-full text-[9px] font-semibold uppercase tracking-wider font-mono ${
                    ...
                    <td className="px-5 py-4 text-slate-300 max-w-xs leading-relaxed">
                      {req.reason}
                    ...
                    <td className="px-5 py-4 text-right print:hidden">
                      {req.file_urls && req.file_urls.length > 0 ? (
                        <button
                          ...
                          className="px-3 py-1 rounded-full bg-blue-600/10 hover:bg-blue-600/20 text-blue-400 border border-blue-500/30 text-[11px] font-medium transition active:scale-[0.98]"
                        >
